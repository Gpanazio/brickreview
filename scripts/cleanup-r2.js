...
const ALLOWED_PREFIXES = ["files/", "videos/", "proxies/", "thumbnails/", "project-covers/"];
...
if (DATABASE_URL) {
  ...
  console.warn(
    "⚠️  AVISO: DATABASE_URL não encontrada. A verificação do Banco de Dados será ignorada."
  );
}
  ...
  try {
    while (isTruncated) {
      ...
      if (isTruncated) {
        ...
        console.log(
          "...continuando listagem, token: " +
            (continuationToken ? continuationToken.substring(0, 10) : "undefined") +
            "..."
        );
      }
    }
    ...
    console.log(
      "✅ Listagem completa. Encontradas " +
        allKeys.size +
        " chaves R2 válidas (filtradas por prefixo)."
    );
    return allKeys;
  } catch (error) {
...
async function cleanupR2() {
  ...
  if (!dbPool && r2Keys.size > 0) {
    console.warn(
      "⚠️  AVISO: Banco de dados não acessível. Não é possível determinar quais arquivos apagar. Listando apenas arquivos R2 dentro dos prefixos permitidos."
    );
