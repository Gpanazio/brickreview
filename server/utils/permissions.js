...
export async function requireProjectAccess(req, res, _projectId) {
  if (!req.user?.id) {
...
export async function requireProjectAccessFromVideo(req, res, videoId) {
  const videoResult = await query("SELECT project_id FROM brickreview_videos WHERE id = $1", [
    videoId,
  ]);
...
export async function requireProjectAccessFromFolder(req, res, folderId) {
  const folderResult = await query("SELECT project_id FROM brickreview_folders WHERE id = $1", [
    folderId,
  ]);
...
export async function requireProjectAccessFromFile(req, res, fileId) {
  const fileResult = await query("SELECT project_id FROM brickreview_files WHERE id = $1", [
    fileId,
  ]);
